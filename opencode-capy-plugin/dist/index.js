...
export const CapyPlugin = async (ctx) => {
    return {
        tool: {
            capy_get_preview_brief: tool({
                description: "Get a preview brief for the current project. Returns repo map, inspection plan, constraints, and exact /preview spec the agent should follow before writing code. Call this first for any UI preview, design-system, component-catalog, or style-audit task.",
                args: {
                    task: tool.schema
            ...
            capy_get_design_system: tool({
                description: "Build and write a stable machine-readable design-system artifact for the current project. Use this when you need a durable JSON source of truth for UI work.",
                args: {
            ...
            capy_update_preview: tool({
                description: "Diff the current project against Capy's last snapshot, refresh the design-system artifact, and return an incremental /preview update brief. Works even without git.",
                args: {
            ...
            capy_detect_framework: tool({
                description: "Detect the framework (Next.js, React Router, etc.) and routing style of the current project.",
                args: {},
                async execute(args, context) {
                    const { directory } = context;
                    const capy = await getCapyMcp();
