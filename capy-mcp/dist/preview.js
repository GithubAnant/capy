...
import { buildArtifactIgnorePatterns, buildStateHashes, ensureArtifactsStructure, loadArtifactState, writeArtifactState, } from "./artifacts.js";
import { appendGitignorePatterns, relativeImportPath, stableStringify, writeTextIfChanged } from "./files.js";
import { detectFramework } from "./framework.js";
export async function generatePreview(projectRoot, config, designSystem, options = {}) {
    ...
    if (framework.kind === "react-router") {
        ...
        const registryModule = buildPreviewRegistryModule(data, framework, projectRoot, paths.generatedDir);
        const files = await Promise.all([
            ...
            appendGitignorePatterns(projectRoot, buildArtifactIgnorePatterns(projectRoot, config).concat("src/capy-preview")),
        ]);
        ...
        return {
            ...
            framework: {
                ...framework,
                confirmationMessage: "React Router route module generated at .capy/generated/react-preview-route.tsx. Confirm the router mount point before wiring /preview automatically.",
            },
            route: config.previewRoute,
            files,
            warnings: [
    ...
    await persistState(projectRoot, config, framework, files, data);
    const status = files.some((file) => file.changed) ||
...
function buildPreviewData(designSystem, framework, config, previousState) {
    const warnings = [];
    const components = designSystem.components.map((component) => buildRenderableComponent(component, framework, warnings));
    ...
    const fingerprint = createPreviewFingerprint(framework.kind, config.previewRoute, config.previewLayout, foundations, components, boards, uniqueWarnings);
    const generatedAt = previousState?.hashes.previewFingerprint === fingerprint
...
function buildRenderableComponent(component, framework, warnings) {
    const exampleProps = inferExampleProps(component);
    const hasEnoughInferredProps = component.requiredProps.every((prop) => prop in exampleProps);
    ...
    if (framework.kind === "next-app-router" && !component.isClientComponent) {
        warnings.push("Next App Router preview auto-renders client components only. Server-default components stay as metadata cards until examples are added or the component is client-safe.");
        return {
            name: component.name,
            filePath: component.filePath,
            exportType: component.exportType,
...
function inferExampleProps(component) {
    const props = {};
    ...
    for (const prop of component.requiredProps) {
        ...
        if (normalized === "children")
            ...
            props[prop] = component.name;
        else if (normalized.includes("title"))
            props[prop] = `${component.name} title`;
        else if (normalized === "open" || normalized.endsWith("open"))
            props[prop] = true;
        else if (normalized.includes("checked") || normalized.includes("selected"))
            props[prop] = true;
        else if (normalized.includes("count"))
            props[prop] = 3;
        else if (normalized.includes("variant"))
            props[prop] = "primary";
        else if (normalized.includes("items") || normalized.includes("options"))
            props[prop] = [];
        else if (normalized.includes("placeholder"))
            props[prop] = `Preview ${component.name}`;
        else if (normalized.includes("value"))
            props[prop] = name.includes("input") ? "Preview value" : component.name;
    }
...
function buildBoards(components) {
    const boards = [
        {
            id: "foundations",
            title: "Foundations",
...
function buildPreviewRegistryModule(data, framework, projectRoot, generatedDir) {
    ...
    const imports = importableComponents
        ...
        const importPath = relativeImportPath(generatedDir, join(projectRoot, component.filePath));
    ...
    const entries = data.components
        .map((component) => {
        const index = importableComponents.findIndex((candidate) => candidate.name === component.name && candidate.filePath === component.filePath);
...
function buildRenderExpression(component, alias, framework) {
    ...
    const frameworkNote = framework.kind === "next-app-router"
...
function buildPreviewPageModule() {
    return `"use client";
...
function buildNextRouteModule(framework, previewPagePath) {
...
function buildReactRouteModule() {
...
async function persistState(projectRoot, config, framework, files, data) {
    const state = {
        version: "2",
        framework: framework.kind,
        route: config.previewRoute,
        ...
        hashes: buildStateHashes(Object.fromEntries(files.map((file) => [file.path, `${file.changed}:${file.created}`]))),
        warnings: data.meta.warnings,
        missingExamples: collectMissingExamples(data.components),
    };
    state.hashes.previewFingerprint = createPreviewFingerprint(framework.kind, config.previewRoute, config.previewLayout, data.foundations, data.components, data.boards, data.meta.warnings);
    await writeArtifactState(projectRoot, config, state);
}
function collectMissingExamples(components) {
    return components
        .filter((component) => component.renderMode === "example")
        .map((component) => component.name);
}
function summarizePreview(components) {
...
function createPreviewFingerprint(framework, route, layout, foundations, components, boards, warnings) {
