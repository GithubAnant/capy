...
import { ensureDir, fileExists, hashContent, readText, stableStringify, toPosixPath, writeTextIfChanged, } from "./files.js";
export function getArtifactPaths(projectRoot, config) {
...
export async function ensureArtifactsStructure(projectRoot, config) {
...
export async function persistDesignSystemArtifact(projectRoot, config, designSystem) {
    const paths = getArtifactPaths(projectRoot, config);
...
export async function loadArtifactState(projectRoot, config) {
...
export async function writeArtifactState(projectRoot, config, state) {
    const { statePath } = getArtifactPaths(projectRoot, config);
    await ensureDir(join(projectRoot, config.artifactsDir));
    await writeTextIfChanged(statePath, `${stableStringify(state)}\n`);
}
export async function ensureExamplesTemplate(projectRoot, config) {
...
export function buildArtifactIgnorePatterns(projectRoot, config, previewRoutePath) {
    const base = [
...
export function buildStateHashes(entries) {
    return Object.fromEntries(Object.entries(entries).map(([key, value]) => [key, hashContent(value)]));
}
