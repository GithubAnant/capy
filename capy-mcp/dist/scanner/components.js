...
function analyzeComponent(file, content, relPath, fallbackName) {
    ...
    const sourceFile = ts.createSourceFile(file, content, ts.ScriptTarget.Latest, true, extname(file).toLowerCase() === ".jsx" ? ts.ScriptKind.JSX : ts.ScriptKind.TSX);
    ...
    for (const statement of sourceFile.statements) {
        ...
        if (ts.isVariableStatement(statement) && hasExportModifier(statement)) {
            for (const declaration of statement.declarationList.declarations) {
                ...
                if (!isComponentName(detectedName))
                    continue;
                exportType = hasDefaultModifier(statement) ? "default" : "named";
                importName = detectedName;
...
function getVariableHints(declaration) {
...
function getParameterHints(parameter) {
    if (!parameter) {
        return { requiredProps: [], canRenderWithoutProps: true };
    }
