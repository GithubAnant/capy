...
export async function getDesignSystemContext(projectRoot: string): Promise<{
  config: CapyConfig;
  ...
  const designSystemArtifact = await persistDesignSystemArtifact(
    projectRoot,
    config,
    designSystem
  );
...
export async function updateProjectArtifacts(
  ...
  return {
    status:
      configChanged || designSystemArtifact.status === "updated" || preview.status === "updated"
