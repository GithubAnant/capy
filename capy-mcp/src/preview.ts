...
function buildHTML(ds: DesignSystem): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
function renderColors(ds: DesignSystem): string {
  if (ds.colors.length === 0) return `<div class="section"><h2>Colors</h2><p class="empty">No colors found</p></div>`;
  const cards = ds.colors
    .map(
      (c) => `<div class="color-card">
...
function renderTypography(ds: DesignSystem): string {
  if (ds.typography.length === 0) return `<div class="section"><h2>Typography</h2><p class="empty">No typography tokens found</p></div>`;
  const items = ds.typography
    .map(
      (t) => `<div class="typo-item"><span class="typo-name">${esc(t.name)}<span class="badge">${t.source}</span></span><span class="typo-value">${esc(t.value)}</span></div>`
    )
    .join("\n");
  return `<div class="section"><h2>Typography (${ds.typography.length})</h2>${items}</div>`;
}

function renderSpacing(ds: DesignSystem): string {
  if (ds.spacing.length === 0) return `<div class="section"><h2>Spacing</h2><p class="empty">No spacing tokens found</p></div>`;
...
function renderComponents(ds: DesignSystem): string {
  if (ds.components.length === 0) return `<div class="section"><h2>Components</h2><p class="empty">No components found</p></div>`;
  const items = ds.components
    .map(
      (c) => `<div class="comp-item"><span class="comp-name">${esc(c.name)}</span><span class="badge">${c.exportType}</span><br><span class="comp-path">${esc(c.filePath)}</span></div>`
    )
    .join("\n");
  return `<div class="section"><h2>Components (${ds.components.length})</h2>${items}</div>`;
}

function renderCSSVariables(ds: DesignSystem): string {
  if (ds.cssVariables.length === 0) return "";
  const rows = ds.cssVariables
    .map(
      (v) => `<tr><td>${esc(v.name)}</td><td>${esc(v.value)}</td><td>${esc(v.file)}</td></tr>`
    )
    .join("\n");
