 ...
 */
function toSafeRelativeDir(dir: string, projectRoot: string): string | null {
...
async function scanForComponents(
  ...
  for (const dir of rawDirs) {
    ...
    if (safe !== null) safeDirs.add(safe);
  }

  if (safeDirs.size === 0) return [];

  const patterns = Array.from(safeDirs).map(
