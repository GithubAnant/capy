...
function isColor(name: string, value: string): boolean {
  ...
  if (colorNames.some((n) => name.includes(n))) return true;
  if (/^#[0-9a-f]{3,8}$/i.test(value)) return true;
  if (/^(rgb|hsl|oklch)\(/.test(value)) return true;
  return false;
}

function isSpacing(name: string, value: string): boolean {
  const spacingNames = ["spacing", "gap", "margin", "padding", "size", "width", "height", "radius"];
  if (spacingNames.some((n) => name.includes(n))) return true;
